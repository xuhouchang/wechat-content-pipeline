#!/usr/bin/env python3
"""
Save step for blog items after agent filtering.
...
def main():
    ...
    raw_dir = TMP_DIR / "blogs"
    if not raw_dir.exists():
        print("No blogs tmp dir"); sys.exit(0)

    filtered_files = sorted(raw_dir.glob(f"blogs_filtered_{date_str}.json"))
    raw_files = sorted(raw_dir.glob(f"blogs_raw_{date_str}.json"))
...
"""
    ...
    update_daily_summary(date_str, "Part 2: AI Company Blogs", {"saved": saved})
    print(f"\n  Done: {saved} blog items saved")
